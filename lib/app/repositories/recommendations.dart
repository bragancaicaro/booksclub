import 'dart:convert';
import 'package:booksclub/app/models/recommendation.dart';
...
class RecommendationsManager {
  ...
  RecommendationsManager(this.initialUrl);

  bool get isLoading => _isLoading;

  List<Recommendation> get items => List.from(_items);
    ...
    if (response.statusCode == 200) {
      ...
      if (data.containsKey('results') && data['results'] is List) {
        ...
        if (results.isNotEmpty) {
          List<Recommendation> newItems =
              results.map((item) => Recommendation.fromJson(item)).toList();
